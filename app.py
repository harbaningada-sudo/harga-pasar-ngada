...
if not df_harga.empty:
    if pilihan == "🏠 Dashboard Beranda":
        st.markdown('<div class="hero-section"><h1>Smart Economy Ngada 👋</h1><p>Implementasi Orientasi Pelayanan melalui transparansi harga pasar.</p></div>', unsafe_allow_html=True)
        search = st.text_input("🔍 Cari komoditas (Contoh: Beras, Ayam)...", "")
        df_show = df_harga.copy()
        if search:
            mask = (df_show['KOMODITAS'].str.contains(search, case=False, na=False)) | (df_show['KATEGORI_INDUK'].str.contains(search, case=False, na=False))
        ...
        for _, row in df_show.iterrows():
            ...
            st.markdown(f'<div class="card-container {css}"><div style="display: flex; justify-content: space-between; align-items: center;"><div><b>{row["KOMODITAS"]}</b><br><small>Satuan: {row["SATUAN"]}</small></div><div style="text-align: right;"><span class="price-main">Rp {h_ini:,}</span><br><span style="color: {warna}; font-weight: 700;">{ikon} {ket}</span><br><span style="font-size:0.8rem; color:gray;">Kemarin: Rp {h_kmrn:,}</span></div></div></div>', unsafe_allow_html=True)

    # --- MENU 2: TREN HARGA (DENGAN FILTER PILIHAN) ---
    elif pilihan == "📈 Tren Harga Komoditas":
        st.title("📈 Analisis Perbandingan Harga")
        st.markdown("Pilih komoditas yang ingin Anda pantau perkembangannya.")
        
        # Ambil daftar komoditas yang valid (bukan judul kategori)
        df_valid = df_harga.dropna(subset=['SATUAN'])
        list_komoditas = df_valid['KOMODITAS'].unique().tolist()
        
        # Pilihan Multiselect
        pilihan_user = st.multiselect(
            "Silakan pilih satu atau beberapa komoditas:",
            options=list_komoditas,
            placeholder="Ketik nama bahan pangan..."
        )
        
        if pilihan_user:
            df_filtered = df_valid[df_valid['KOMODITAS'].isin(pilihan_user)]
            # Melt data agar cocok untuk grafik perbandingan
            df_plot = df_filtered.melt(id_vars=['KOMODITAS'], value_vars=['HARGA KEMARIN', 'HARGA HARI INI'], var_name='Waktu', value_name='Harga (Rp)')
            ...
            fig = px.bar(
                ...
                title="Perbandingan Harga Kemarin vs Hari Ini"
            )
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.warning("Silakan pilih komoditas pada kolom di atas untuk memunculkan grafik.")
        ...
        st.markdown('<div class="card-container"><h3>Integritas & Pelayanan Prima</h3><p>Wujud nyata dedikasi ASN Bagian Perekonomian & SDA Kabupaten Ngada dalam transformasi digital.</p></div>', unsafe_allow_html=True)
