...
st.markdown("""
    ...
    .link-tombol:hover { background-color: #E0E7FF; }
...
with st.sidebar:
    ...
    st.divider()
    pilihan = st.radio("Navigasi:", ["🏠 Dashboard Beranda", "📈 Tren Harga", "📰 Berita & Media", "📥 Unduh Data", "ℹ️ Informasi Layanan"])
...
if data_ok:
    if pilihan == "🏠 Dashboard Beranda":
        ...
        for _, row in df_harga.iterrows():
            st.markdown(f'<div class="card-container"><b>{row["KOMODITAS"]}</b><br><span style="color:#059669; font-size:1.2rem; font-weight:800;">Rp {int(row["HARGA HARI INI"]):,}</span></div>', unsafe_allow_html=True)
        ...
        st.markdown("Klik pada tombol di setiap berita untuk melihat foto atau dokumen pendukung.")
        st.divider()
        
        # Berita terbaru muncul paling atas
        for _, row in df_berita.iloc[::-1].iterrows():
            with st.container():
                ...
                # Tampilkan Tombol Link jika ada isinya dan diawali http
