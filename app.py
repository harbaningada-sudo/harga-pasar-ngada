...
# --- 7. FUNGSI TAMPILAN HARGA (VERSI BERSIH TANPA BOCOR) ---
def get_price_html(label, ini, kmrn):
    diff = ini - kmrn
    ...
    return f"""
    <div style="flex: 1; min-width: 140px; padding: 10px; background: #f8fafc; border-radius: 10px; margin: 5px;">
        <div style="font-weight: bold; font-size: 0.8rem; color: #0369a1; border-bottom: 1px solid #e2e8f0; margin-bottom: 5px;">🏪 {label}</div>
        <div style="font-size: 0.7rem; color: #64748B;">Hari Ini:</div>
        <div style="font-size: 1.1rem; font-weight: bold; color: {clr};">Rp {ini:,}</div>
        <div style="font-size: 0.7rem; color: #64748B; margin-top: 3px;">Kemarin:</div>
        <div style="font-size: 0.8rem; color: #94A3B8; text-decoration: line-through;">Rp {kmrn:,}</div>
    ...
    if not df_harga.empty:
        ...
        for _, r in filtered.iterrows():
            if r['SATUAN'] == 0 or str(r['SATUAN']) == "0":
                ...
                h_besar = get_price_html("PEDAGANG BESAR", r['B_INI'], r['B_KMRN'])
                h_kecil = get_price_html("PEDAGANG KECIL", r['K_INI'], r['K_KMRN'])
                
                # Menggunakan baris Flex sederhana tanpa div penutup tambahan yang berisiko bocor
                st.markdown(f"""
                <div class="price-card">
                    <div style="display: flex; flex-wrap: wrap; align-items: flex-start;">
                        <div style="flex: 1.5; min-width: 180px; margin-bottom: 10px;">
...
# --- 9. HALAMAN POTENSI (DENGAN FOTO) ---
    ...
    with t1:
        c1, c2 = st.columns(2)
        with c1:
            if os.path.exists("cengkeh.jpeg"): st.image("cengkeh.jpeg", caption="Potensi Cengkeh")
        with c2:
