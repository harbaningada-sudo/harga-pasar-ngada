...
st.markdown(f"""
    ...
    .flex-container {{ display: flex; justify-content: space-between; gap: 15px; }}
    .stButton button {{
...
with c2:
    ...
    st.markdown("<p style='color:#0369a1; font-weight:bold; margin:0; font-size:1.2rem;'>Bagian Perekonomian & SDA Setda</p>", unsafe_allow_html=True)
...
def format_price_ui(ini, kmrn):
    ...
    return f"""
        <div style="line-height:1.2;">
            ...
            <div style="font-size: 0.7rem; color: #94A3B8;">Kemarin: Rp {kmrn:,}</div>
            <div style="margin-top: 5px; padding: 2px 6px; border-radius: 4px; background: {color}15; display: inline-block;">
                <span style="color:{color}; font-weight:bold; font-size: 0.7rem;">{icon} {status}</span>
            </div>
        </div>
    """
...
if st.session_state.page == "Beranda":
    ...
    if not df_harga.empty:
        ...
        for _, r in filtered.iterrows():
            if r['SATUAN'] == 0 or str(r['SATUAN']) == "0":
                st.markdown(f"<div style='background:#0369a1; color:white; padding:5px 15px; border-radius:8px; margin-top:20px;'>🔍 {r['KOMODITAS']}</div>", unsafe_allow_html=True)
            else:
                st.markdown(f"""
                <div class="price-card">
                    <div class="flex-container">
                        <div style="flex: 1.2;">
                            ...
                            <div style="font-size: 0.8rem; color: #64748B;">Satuan: {r['SATUAN']}</div>
                        </div>
                        <div style="flex: 1; border-left: 1px solid #eee; padding-left: 10px;">
                            <div style="font-size: 0.65rem; font-weight: bold; color: #475569;">PEDAGANG BESAR</div>
                            {format_price_ui(r['B_INI'], r['B_KMRN'])}
                        </div>
                        <div style="flex: 1; border-left: 1px solid #eee; padding-left: 10px;">
                            <div style="font-size: 0.65rem; font-weight: bold; color: #475569;">PEDAGANG KECIL</div>
    ...
    with tab1:
        ...
        with c_a: 
            if os.path.exists("cengkeh.jpeg"): st.image("cengkeh.jpeg", caption="Hasil Bumi Ngada")
        with c_b:
    ...
    with tab2:
        ...
        with c_d:
            if os.path.exists("17 pulau riung.webp"): st.image("17 pulau riung.webp", caption="Riung 17 Pulau")
    ...
    st.markdown(f"### Tentang Kami\n{store['about_text']}")

elif st.session_state.page == "Unduh":
    st.download_button("📥 Download Data CSV", df_harga.to_csv(index=False), "data_ekonomi_ngada.csv", use_container_width=True)
